...
public class PlaylistFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    	...
    	if (savedInstanceState == null) {
            ...
            adapter = new PlaylistAdapter(activity, this, selected, manager, LEVEL_NUMBER);
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
	  if (v.getId()==R.id.playlistView) {
		  ...
		  menu.add(Menu.NONE,0,0,"Show Item");
		  menu.add(Menu.NONE,1,0,"Remove Item");
	  }
	}
	...
	void load_playlist() {
		...
		if (activity.playlistid>0) {
			...
			playlistmap.put((long) 0,"No playlist loaded\nLoad one from the menu");
		}
        adapter = new PlaylistAdapter(activity, this, selected, manager, LEVEL_NUMBER);
    	treeView.setAdapter(adapter);
	...
	void add_playlist(TreeBuilder<Long> treeBuilder, int playlistid,int level) {
        ...
        for (int i=0; i<jArray.length();i++) {
        	try {
        		...
        		treeBuilder.sequentiallyAddNextNode(results.getLong("playorder"),level);
        		if (results.getString("type").equals("play") || results.getString("type").equals("sub")) {
        			...
        			if (pArray != null) {
        				playlistmap.put(results.getLong("playorder"),pArray.getJSONObject(0).getString("title"));
        				add_playlist(treeBuilder,results.getInt("data"),newlevel);
        			}
        		} else if (results.getString("type").equals("song")) {
        			String Query2 = "SELECT pagetitle, lyrics FROM page WHERE pageid="+results.getString("data");
        			JSONArray pArray = activity.runQuery("lyricDb", Query2);
        			if (pArray != null) {
        				String[] lines = pArray.getJSONObject(0).getString("lyrics").split("\n");
        				playlistmap.put(results.getLong("playorder"),lines[0]);
        			}
        		} else if (results.getString("type").equals("vers")) {
        			playlistmap.put(results.getLong("playorder"),"Verses "+results.getString("data"));
        		} else if (results.getString("type").equals("file")) {
        			playlistmap.put(results.getLong("playorder"),"File:"+results.getString("data"));
        		} else {
        			playlistmap.put(results.getLong("playorder"), "Unknown item type");
        		}
