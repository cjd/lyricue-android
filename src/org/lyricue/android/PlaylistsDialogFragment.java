...
public class PlaylistsDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        try {
        	...
        	builder.setItems(activity.playlists_text, new DialogInterface.OnClickListener() {				
				@Override
				public void onClick(DialogInterface dialog, int which) {
					Lyricue activity = (Lyricue) getActivity();
					activity.playlistid=activity.playlists_id[which];
					System.err.println("Loading "+activity.playlists_text[which]);
					LyricuePagerAdapter adapter = activity.adapter;
					PlaylistFragment frag = (PlaylistFragment) adapter.getFragment(1);
					frag.load_playlist();
				}
			});
